...
router.post('/login', async (req, res)=>{
    ...
    if(email == await user.email(email) && checkPass && await user.checkRole(email) == 'superadmin'){
        req.session.superadmin = email;
        res.redirect('/');
    } else if (email == await user.email(email) && checkPass && await user.checkRole(email) == 'user') {
        req.session.user = email;
        res.redirect('/');
    } else {
        res.render('login', { title: "Login", loginFail : "Email atau kata sandi salah."})
    }
});
...
router.put('/account/changeemail', 
    body('email').custom(async (value, {req}) => {
        const duplicate = await user.checkDuplicate(value.toLowerCase());
        if (value == req.body.oldEmail){
            throw new Error('Ganti email gagal: email baru tidak boleh sama dengan email sekarang.');
        }
        if (duplicate){
    ...
    async (req, res) => {
        if(req.session.superadmin && req.session.superadmin !== 'superadmin@gmail.com'){
            const errors = validationResult(req);
            if (!errors.isEmpty()) {
                res.render('account', {
                    title: 'Akun',
                ...
                req.session.destroy(function(err){
                    if(err){
                        ...
                        res.render('login', { title: "Login", logout : "Ganti email berhasil, silahkan masuk kembali."})
                    }
                })
            }
        }else{
...
router.put('/account/changepassword', 
    ...
    body('password').custom(async (value, {req}) => {
        checkPass = await bcrypt.compare(value, req.body.matchPass)
        if (checkPass){
            throw new Error('Ganti kata sandi gagal: kata sandi baru tidak boleh sama dengan kata sandi lama.');
        }
        return true;
    }),
    body('confirmPassword').custom(async (value, {req}) => {
        if (value !== req.body.password){
            throw new Error('Ganti kata sandi gagal: kata sandi baru dan konfirmasi kata sandi baru tidak cocok.');
        }
        return true;
    }),
    async (req, res) => {
            ...
            if (!errors.isEmpty()) {
                ...
                req.session.destroy(function(err){
                    if(err){
                        ...
                        res.render('login', { title: "Login", logout : "Ganti kata sandi berhasil, silahkan masuk kembali."})
                    }
                })
            }
        }else if(req.session.user){
            ...
            if (!errors.isEmpty()) {
                ...
                req.session.destroy(function(err){
                    if(err){
                        ...
                        res.render('login', { title: "Login", logout : "Ganti role berhasil, silahkan masuk kembali."})
                    }
                })
            }
        }else{
...
router.post('/barang', upload.single("image"), 
    ...
    async (req, res) => {
        if(req.session.superadmin){
            ...
            if (!errors.isEmpty()) {
                ...
                await stok.addBarang(namabarang,deskripsi,stock,image,penginput,kodebarang)
...
router.put('/barang/:id', upload.single("image"), 
            ...
            if (!errors.isEmpty()) {
                ...
                if(req.file){
                    if(req.file.filename){
                        ...
                        await stok.updateBarang(namabarang,deskripsi,stock,image,kodebarang,idbarang)
...
router.post('/barangmasuk', async (req, res) => {
    if(req.session.superadmin){
        ...
        await bmasuk.addBarangMasuk(idbarang,keterangan,qty,namabarang_m,penginput,kodebarang_m)
            ...
            if(qtyy>currentQty){
                ...
                if(kurangin<0){
                    ...
                    res.render('barangMasuk', {
                        ...
                        brgm: barangMasuk,
                        moment,
                        deleteFail : "Edit barang masuk gagal: mengurangi jumlah masuk barang ini akan mengakibatkan nilai stok barang menjadi negatif.",
                        brg: barang
                    })
                } else {
        ...
        if(stockk != 'undefined') {
            ...
            if(selisih<0){
                ...
                res.render('barangMasuk', {
                    ...
                    brgm: barangMasuk,
                    moment,
                    deleteFail : "Hapus barang masuk gagal: menghapus barang masuk ini akan mengakibatkan nilai stok barang menjadi negatif.",
                    brg: barang
                })
            } else {
...
router.post('/barangkeluar', async (req, res) => {
    if(req.session.superadmin){
        ...
        if(newStock<0){
            ...
            await stok.updateStock(newStock, idbarang)
            await bkeluar.addBarangKeluar(idbarang,penerima,qty,namabarang_k,penginput,kodebarang_k)
            ...
            if(qtyy>currentQty){
                ...
                if(kurangin<0){
                    ...
                    res.render('barangKeluar', {
                        ...
                        brgk: barangKeluar,
                        moment,
                        error : "Edit barang keluar gagal: menambah jumlah keluar barang ini akan mengakibatkan nilai stok barang menjadi negatif.",
                        brg: barang
                    })
                } else {
                    await stok.updateStock(kurangin, idbarang)
